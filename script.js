...
inputBox.onkeyup = ()=>{
    ...
    if(userTask.trim() != 0){
        ...
        // if user entered value is less than or equal to 28
        if(userTask.length <= 28){
        ...
        // if user entered value is more than 28
        else if(userTask.length > 28){
            if(event.keyCode == 13){ // if enter key is pressed
...
deleteAllBtn.onclick = ()=>{
    if(confirm("You are about to clear all tasks!\nProceed?")){ // if pop-up is confirmed
        let getLocalStorage = localStorage.getItem("OneDay | To Do"); // getting localStorage
        if(getLocalStorage == null){ // if localStorage has no data
            listArr = []; // create a blank array
        }else{
...
// Notepad function
function notepad() {
    let notepad = document.getElementById('notes'); // selecting notepad div
